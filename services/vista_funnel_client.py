...
class VistaFunnelClient:
    ...
    def fetch_created_deals(
        ...
        broker_names = self._fetch_user_names(
            {
                str(deal["commercial_broker_id"]).strip()
                for deal in deals
                if deal.get("commercial_broker_id") not in (None, "")
            }
        )
        for deal in deals:
            broker_id = str(deal.get("commercial_broker_id") or "").strip()
            broker_name = broker_names.get(broker_id)
            deal["commercial_broker_name"] = broker_name
            if not deal.get("responsible") and broker_name:
                deal["responsible"] = broker_name

        return deals

    def _fetch_user_names(self, broker_ids: set[str]) -> Dict[str, str]:
        """Resolve Vista broker IDs without exposing contact or client data."""
        if not broker_ids:
            return {}
        ...
        while True:
            ...
            try:
                ...
                # Name resolution enriches attribution but must never make the
                # verified funnel totals unavailable.
                return names
