...
function fallbackAnswerFromTool(payload) {
  ...
  if (value.group_by === "equipe") {
    ...
    if (!breakdown.length) {
      return [
        "Para gestão atual, não foi possível separar as propostas por equipe porque nenhum dos negócios em Proposta retornou uma equipe ou um responsável com vínculo gerencial válido.",
        Number.isFinite(openTotal) && Number.isFinite(currentTotal)
          ? `Totais confiáveis: ${openTotal} em aberto e ${currentTotal} atualmente na etapa Proposta.`
          : "",
    ...
    return [
      "Para gestão atual, a distribuição das propostas por equipe é:",
      rows.join("\n"),
      coverageText,
  ...
  if (Number.isFinite(openTotal) && Number.isFinite(currentTotal)) {
    return `Para gestão atual, há ${openTotal} negócios com status “Em aberto” na etapa Proposta e ${currentTotal} negócios atualmente nessa etapa. Esses são os números operacionais confiáveis da fotografia atual; eles não representam todas as entradas históricas em Proposta porque esse histórico não está disponível.`;
  }
  return "";
}
...
async function chat(request, env) {
  ...
  const answer = outputText(payload) || fallbackAnswerFromTool(payload);
  if (!answer) {
